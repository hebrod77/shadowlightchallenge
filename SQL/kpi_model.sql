create or replace view v_ads_spend as
select date, platform, account, campaign, country, device, spend, clicks, impressions, 
conversions, round(spend/conversions,2) as CAC, round(conversions * 100 / spend,2) as ROAS
from ads_spend_data;

create or replace view v_kpi_model as
with max_date as (
  select max(date) as max_date from v_ads_spend
), 
last_30_days as (
